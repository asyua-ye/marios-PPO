...
class SQ(nn.Module):
    def __init__(self, cnn, action_dim):
        ...
        self.cnn = cnn
        self.l1 = nn.Linear(self.cnn.feature_size + action_dim, 512)
        self.q1 = nn.Linear(512, 1)
        
        self.l2 = nn.Linear(self.cnn.feature_size + action_dim, 512)
...
class agent(object):
    def __init__(self,state_dim, action_dim,hp) -> None:
        ...
        self.SQ = SQ(self.cnn_net,action_dim).to(self.device)
        ...
        self.sq_optimizer = torch.optim.Adam(self.SQ.parameters(), lr=hp.critic_lr,eps=hp.eps)
        ...
        self.scheduler = [LambdaLR(self.actor_optimizer, lr_lambda=lambda_lr),
                          ...
                          LambdaLR(self.sq_optimizer, lr_lambda=lambda_lr)]
    ...
    def train(self,sample,process,writer):
        ...
        q1,q2 = self.SQ.getQ(states,action)
        ...
        self.sq_optimizer.zero_grad()
        q_loss.backward()
        torch.nn.utils.clip_grad_norm_(list(self.cnn_net.parameters()) + list(self.SQ.parameters()), self.grad)
        self.sq_optimizer.step()
        ...
        value_loss.backward()
        torch.nn.utils.clip_grad_norm_(list(self.cnn_net.parameters()) + list(self.critic.parameters()), self.grad)
        ...
        actor_loss.backward()
        torch.nn.utils.clip_grad_norm_(list(self.cnn_net.parameters()) + list(self.actor.parameters()), self.grad)
