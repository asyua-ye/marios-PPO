...
def actor(hp, replaybuffer, event, counter):
    ...
    while True:
        if t >= 0:
            ...
            states, actions, action_log_probs,returns,rewards,next_states,masks = rollout.computeReturn(next_value, mask)
            data = (np.copy(states), np.copy(actions), np.copy(action_log_probs), np.copy(returns), np.copy(rewards),
                    np.copy(next_states), np.copy(masks))
        ...
        while True:
            ...
            if count != -1:
                ...
                for _ in range(hp.num_epch_train):
                    sample  = replaybuffer.sample(hp.batch_size, 1)
