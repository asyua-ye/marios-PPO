...
def Reward(info,reward,old_info,done,eight):
    ...
    if info["world"] == 8 and info["stage"] == 4:
        if (info["x_pos"] >=1272 and eight==0):
            eight += 1
        elif (eight == 1 and info["x_pos"]>=350 and old_info["x_pos"]<=350) or (eight == 1 
                        and (info["x_pos"]>390) and info["x_pos"]<=1272):
            eight += 1
            p = -info['score']/10 - 50
            new_done = True
    ...
    if info["world"] == 7 and info["stage"] == 4:
        if (506 <= info["x_pos"] <= 832 and info["y_pos"] > 127) or (
                ...
                2114 < info["x_pos"] < 2440 and info["y_pos"] < 191) or info["x_pos"] < old_info['x_pos'] - 500:
            p = -info['score']/10 - 50
            new_done = True
