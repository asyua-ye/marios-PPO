...
class agent(object):
    def __init__(self,state_dim, action_dim,hp) -> None:
        ...
        self.ActorCritic_optimizer = torch.optim.AdamW(self.ActorCritic.parameters(), lr=hp.actor_lr,eps=hp.eps,
    ...
    def learn(self, sample, writer):
        ...
        with torch.no_grad():
            ...
            # action_log_probs = torch.log(probabilities * new_action + (1 - probabilities) * (1 - new_action))
            accuracy = (new_action == label_action).float().mean()
        ...
        return actor_loss.item(),value_loss.item(),accuracy.item()
