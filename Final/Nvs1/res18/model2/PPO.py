...
class agent(object):
    def __init__(self,state_dim, action_dim,hp) -> None:
        ...
        self.actor = Actor(self.cnn_net,self.action_dim,hp.ppg,hp.threshold).to(self.device)
    ...
    def evaluate(self,sample):
        ...
        with torch.no_grad():
            new_action,_ = self.actor.get_action(state)
            # accuracy = (new_action == label_action).float().mean()
            accuracy = torch.all(new_action == label_action,dim=-1).float().mean()
