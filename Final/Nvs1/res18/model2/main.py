...
def train_offline(RL_agent, eval_env, replaybuffer, hp):    
    ...
    for t in range(int(hp.total_timesteps)):
        ## train
        s = time.time()
        total_acc = np.zeros(1, dtype=np.float64)
        for batch in train_iterator:
            acc = RL_agent.Imitate(batch, writer)
            total_acc += acc
        avg_train = total_acc / len(train_iterator)
        log_and_print(texts,f"Epoch {t},  Average acc: {avg_train[-1]*100:.2f}%")
        e = time.time()
        train_time = (e - s)
...
def toTestPPO(RL_agent, replaybuffer, hp):
    env_name = f"SuperMarioBros-1-1-v0"
    ...
    total_acc = np.zeros(1, dtype=np.float64)
    for batch in train_iterator:
        acc = RL_agent.evaluate(batch)
        total_acc += acc
    avg_train = total_acc / len(train_iterator)
    print(f"Average acc: {avg_train[-1]*100:.2f}%")
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        toTestPPO(RL_agent, replaybuffer, hp)
    else:
